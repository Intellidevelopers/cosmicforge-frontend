...
const AiChatbot = () => {
  ...
  const [messages, setMessages] = useState<
    { sender: string; message: string; timeStamp: string; _id?: string }[]
  >(
    (userDiagnosis.diagnosisChat?.messages as {
      sender: string;
      message: string;
      timeStamp: string;
    }[]) ?? [],
    ...
    if (input.trim() && userSocket.connected) {
      if (
        ...
        if (data?.access && countChatForThisMonth.length >= data?.access) {
          ...
          setMessages([
            ...messages,
            { sender: "user", message: input, timeStamp: `${new Date()}` },
            {
              sender: "bot",
              message: `Sorry you are out of free triers for this month.Upgrade  plan  to enjoy using this feature.`,
              timeStamp: `${new Date()}`,
            },
          ]);
      ...
      setMessages([
        ...messages,
        { sender: "user", message: input, timeStamp: Date() },
      ]);
      setInput("");
      setShowLoader(true);
  ...
  useMemo(() => {
    if (userDiagnosis.diagnosisChat?.messages) {
      ...
      if ((subscription?.planName ?? "Free") !== "Premium") {
        setCountChatForThisMonth(
          userDiagnosis.diagnosisChat?.messages?.filter((message) => {
            return (
  ...
  useEffect(() => {
    setMessages([
      ...
      {
        sender: "bot",
        message: `Hello ${user.data?.fullName} i am your AI assistant how may i help you.`,
        timeStamp: Date(),
      },
    ]);
  }, []);
  ...
  return (
    <div className="overflow-hidden relative">
      ...
      {toggleSearch && (
        <div className="absolute  bg-white w-full top-[12%] z-[600] min-h-[350px] p-10 md:flex flex-col place-items-center justify-center">
      ...
      <div className="flex flex-col h-[90%]  bg-gray-100">
        <div className="flex-1 overflow-auto p-4">
          {messages.map((message, _) => (
            <React.Fragment key={message._id}>
              <div
                className={`mb-4 flex ${message.sender === "user" && "justify-self-end"} `}
                ref={lastMessageRef}
              >
                {message.sender == "bot" && (
                  <img
                    src={botImage}
                    alt="Profile"
                    className="inline self-start rounded-[50%]  mr-2 h-8 w-8"
                  />
                )}

                <div
                  className={`inline-block p-3 rounded-lg shadow-lg ${message.sender === "user" ? "bg-cosmic-color-lightBlue text-white" : "bg-white text-black"}`}
                >
                ...
                {message.sender == "user" && (
                  <img
                    src={user.data?.profile?.profilePicture ?? profilePic}
                    alt="Profile"
                    className="inline-flex self-start rounded-[50%]  ml-2 h-8 w-8"
                  />
                )}
              </div>
              {/*message.diagnosis && 
          ...
          <div
            className={`mb-4 flex ${!showLoader && "hidden"} mt-2 `}
            ref={lastMessageRef}
          >
            <img
              src={botImage}
            ...
            />
            <div
              className={`inline-block p-3 rounded-xl w-20 shadow-lg bg-white overflow-hidden`}
            >
        ...
        <div className="flex-none md:p-4  bg-white border-t border-gray-300 relative">
          ...
          <div
            className={` ${!loadingResponse && "hidden"} absolute top-[-5rem] left-[45%] cursor-pointer flex items-center gap-1 bg-white p-2 rounded-md shadow-md`}
            onClick={() => {
              setLoadingResponse(false);
            }}
          >
