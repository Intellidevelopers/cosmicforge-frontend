...
const Chatbot = () => {
  ...
  const [messages, setMessages] = useState<
    { sender: "user" | "bot"; message: string; timeStamp: string }[]
  >([]);
  const [input, setInput] = useState<string>("");

  const userSocket = useSelector((state: RootReducer) => state.socket);

  const userChatBot = useSelector(
    (state: RootReducer) => state.diagnosis.chatBot,
  );
    ...
    if (userChatBot?.messages) {
      ...
      if ((subscription.userSubcription?.planName ?? "Free") !== "Premium") {
        setCountChatForThisMonth(
          userChatBot.messages.filter((message) => {
            return (
    ...
    if (data?.access && countChatForThisMonth.length >= data?.access) {
      ...
      setMessages([
        ...
        },
        {
          sender: "bot",
          message:
            "Sorry you are out of free triers for this month. Upgrade plan to enjoy using this feature.",
          timeStamp: `${new Date(Date.now()).toLocaleDateString("en-US")}`,
        },
      ]);
  ...
  return (
    <div className="w-full relative overflow-y-hidden ">
      ...
      {toggleSearch && (
        <div className="absolute  bg-white w-full       z-[600] min-h-[350px] p-10 md:flex flex-col place-items-center justify-center">
      ...
      <div className=" flex  flex-col h-[80%] overflow-y-auto bg-gray-100   relative">
        <div className="flex-1 overflow-auto p-8">
          {messages.map((data, index) => (
            <div
              key={index}
              className={`mb-4 flex m-4 ${data.sender === "user" && "justify-self-end"} `}
              ref={lastMessageRef}
            >
              {data.sender == "bot" && (
                <img
                  src={robotProfilePic}
                  alt="Profile"
                  className="inline self-end rounded-[50%]  mr-2 h-8 w-8"
                />
              )}
              <div
                className={`inline-block p-3 rounded-lg shadow-lg ${data.sender === "user" ? "bg-cosmic-color-lightBlue text-white" : "bg-white text-black"}`}
              >
              ...
              {data.sender == "user" && (
                <img
                  ...
                  alt="Profile"
                  className="inline-flex self-end rounded-[50%]  ml-2 h-8 w-8"
                />
              )}
            </div>
          ))}
