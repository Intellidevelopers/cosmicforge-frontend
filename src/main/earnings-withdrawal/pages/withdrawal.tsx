...
const Withdrawal = () => {
  ...
  const [withdeawalDetails, setWithdrawalDetails] = useState<{
    amount: string;
    currency: "NGN" | "USD";
  ...
  const walletBallance = useSelector(
    (state: RootReducer) => state.doctorWallet,
  );
  ...
  const [transactionProcessed, setTrasactionProcessed] =
    ...
    (async () => {
      try {
        if (withdeawalDetails.bankCode && withdeawalDetails.accountNumber) {
          const validAccountDetails = await verifyAccount(
            user.data?.token!!,
            withdeawalDetails.accountNumber,
            withdeawalDetails.bankCode,
          );
          setWithdrawalDetails({
            ...withdeawalDetails,
            accountName: validAccountDetails.data.data.account_name,
          });
        }
      } catch (error) {
        setWithdrawalDetails({
          ...withdeawalDetails,
          accountName: "Not a valid account.",
        });
      }
    })();
  }, [withdeawalDetails.bankCode]);
      ...
      <div className="max-w-[800px] h-fit items-center justify-center w-full gap-4 flex flex-col p-4 shadow-md shadow-black rounded-md">
        ...
        <div className="flex justify-around  gap-4 border-b-2 w-full">
          <p
            ...
            className={`font-bold p-2 ${withdrawalMethod === "bank" ? "border-b-2 border-cosmic-primary-color" : "border-none"}`}
          >
            Bank
          </p>
          <p
            onClick={() => setWithdrawalMethod("cryptocurrency")}
            className={`font-bold p-2 ${withdrawalMethod === "cryptocurrency" ? "border-b-2 border-cosmic-primary-color" : "border-none"}`}
          >
            Cryptocurrency
          </p>
        </div>
          ...
          {withdrawalMethod === "bank" ? (
            <div className="w-full space-y-4">
              <div className="flex w-full flex-col">
                ...
                <input
                  className="min-w-full border p-2"
                  value={withdeawalDetails.amount}
                  title="amount"
                  type="text"
                  name="amount"
                  onChange={(e) => {
                    setWithdrawalDetails({
                      ...
                      amount: new Intl.NumberFormat().format(
                        Number(e.target.value.replace(/[a-z A-Z,]/g, "")),
                      ),
                    });
                  }}
                />
              ...
              <div className="flex w-full flex-col">
                ...
                <select
                  className="min-w-full border p-2 text-right"
                  value={withdeawalDetails.currency}
                  onChange={(e) => {
                    setWithdrawalDetails({
                      ...withdeawalDetails,
              ...
              <div className="flex w-full flex-col gap-2">
                <label className="font-bold" htmlFor="account">
                  Account
                </label>
                <input
                  className="min-w-full border p-2"
                  value={withdeawalDetails.accountNumber}
                  title="account no"
                  type="text"
                ...
                <input
                  className="min-w-full border p-2"
                  value={withdeawalDetails.accountName}
                  title="account name"
                  type="text"
          ...
          <button
            ...
            onClick={async () => {
              if (withdrawalMethod === "bank") {
                ...
                const isNotDataComplete = Object.entries(
                  withdeawalDetails,
                ).some(([_, value]) => {
                  return (
                    !value ||
                ...
                if (isNotDataComplete) {
                  setErrorMessage(
                    "Please fill all details as they are required to continue.",
                  );
                  return;
                }

                let amountToWithdraw =
                  Number(withdeawalDetails.amount.replace(/,/g, "")) * 100;

                if (
                  walletBallance &&
                  walletBallance.wallet &&
                  amountToWithdraw > walletBallance.wallet.amount
                ) {
                ...
                try {
                  const withdrawResponse = await withdrawBalance(
                    user.data?.token!!,
                    "nuban",
                    withdeawalDetails.accountName,
                    withdeawalDetails.accountNumber,
                    withdeawalDetails.bankCode,
                    "NGN",
                  ...
                  if (withdrawResponse.status === 200) {
                    ...
                    setLoading(false);
                    store.dispatch(
                      updateDoctorWallet({ wallet: withdrawResponse.data }),
                    );
                  }
                } catch (error: any) {
                  setLoading(false);
