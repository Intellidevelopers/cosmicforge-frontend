...
const AddCard = () => {
  ...
  const isFormInvalid = () => {
    if (
      cardDetails.cardNumber !== 1234 &&
      cardDetails.cardName !== "John Doe" &&
      cardDetails.cvv !== 123 &&
      cardDetails.expDate !== null
    ) {
      return false;
    } else {
      return true;
    ...
    <>
      ...
      <div className="md:ms-[250px] flex flex-col bg-white p-8 space-y-8 h-[85%]">
        ...
        <div className="flex w-full flex-col gap-4">
          <div>
            <label htmlFor="cardDetails.cardNumber" className="font-bold">
              Card NO
            </label>
            <input
          ...
          <div className="flex w-full gap-8">
            <div className="flex flex-col">
              <label htmlFor="expDate" className="font-bold">
                EXP Date
              </label>
              <input
                onChange={handleChange}
          ...
          <div className="flex justify-between">
            ...
            <div
              className={
                "rounded-full border-2 min-w-8 h-4 " + (saveCard && "bg-black")
              }
              onClick={() => {
            ...
            >
              <div
                className={`rounded-[50%]  w-3 h-3 ${saveCard ? "translate-x-[100%] bg-white" : "bg-black"}`}
              ></div>
            </div>
          </div>
        </div>
        <button
          type="button"
          className={`rounded-md self-center justify-self-end text-white  p-2 w-[250px] ${cardDetails === null ? " bg-gray-500 " : " bg-cosmic-primary-color "}`}
          onClick={() => {
