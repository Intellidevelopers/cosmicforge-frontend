...
const CheckOutPage = () => {
  ...
  const handleClick = (method: string) => {
    if (method === "Card") {
      setCheckOutOption("Card");
    }
    if (method === "Transfer") {
      setCheckOutOption("Transfer");
    }
  };
  ...
  return (
    <>
      ...
      <div className=" p-4 flex flex-col  gap-4">
        ...
        <button
          type="button"
          className="rounded-md text-white self-center justify-self-end font-bold bg-cosmic-primary-color p-2 w-[250px]"
          onClick={() => {
            navigate(navigateToPath);
          }}
        >
          {checkOutOption === "Card" ? "Pay Now" : "I have made this transfer"}
        </button>
      </div>
    </>
  );
