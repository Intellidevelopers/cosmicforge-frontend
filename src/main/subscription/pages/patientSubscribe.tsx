...
const PatientSubscribe = () => {
  ...
  useMemo(() => {
    if (subcription) {
      const formatedSubscriptionDetails =
        subcription.generalSubscriptionDetails?.map((plan) => {
          return {
            name: plan.name,
  ...
  return (
    <div>
      ...
      <p
        className="m-5 bg-white w-fit p-3 shadow-md rounded-md cursor-default"
        onClick={() => {
          dispatch(
            updateCurrencyMode({
              currentCurrencyMode:
      ...
      <div
        className={`${openAlertModal ? "flex" : "hidden"} absolute h-screen w-screen bg-black bg-opacity-15 top-0   md:ps-[25%] justify-center md:justify-normal place-items-center  cursor-default`}
      >
