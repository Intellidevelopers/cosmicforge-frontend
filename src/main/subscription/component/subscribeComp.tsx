...
const SubscribeComp = ({plan,onAlertScreenPopUp }:SubscribeCompProps) => {
 ...
 const {name, message, active, price, duration, offers, commission,usdPrice} = plan
  ...
  return (
    ...
    <div className={`w-[300px] min-h-[600px] 
      ...
      <div className="h-full mt-2">
        ...
        <p className="mt-4 font-bold text-lg">{subscriptionCurrencyMode=== 'NGN' ? 'N':'$'}   {subscriptionCurrencyMode === 'NGN' ? price :usdPrice}<span className="text-sm font-regular font-light"> /{duration}</span></p>
        {
    ...
    {
      subscriptionCurrencyMode === 'USD' && <button  className={`opacity-100 ${active ? 'bg-white text-cosmic-primary-color' : 'bg-cosmic-primary-color text-white'} p-2 w-[150px] font-bold justify-self-end self-center rounded-md shadow-xl`} onClick={()=>{

       if(subscriptionCurrencyMode === 'USD' && !active){
        onAlertScreenPopUp()
       }
    }}>{active ? 'Active' :  'Choose'} plan</button>
    }

     {
       subscriptionCurrencyMode === 'NGN'  && <PaystackButton  disabled={active }    publicKey="pk_live_395226088f830b6c1369b497d49864128c7992dd"  onSuccess={async(e)=>{
