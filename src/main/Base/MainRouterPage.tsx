...
const MainRouterPage = () => {
  ...
  const [isNewCall, setNewCall] = useState<boolean>(
  ...
  useEffect(() => {
      ...
      (async () => {
        try {
          const result = await validateUserSession({
            isKeepMeSignedIn: user.keepMeSignedIn!!,
    ...
    if (user.isAunthenticated && !userSocket.connected) {
      ...
      socket.on("connect", async () => {
        ...
        try {
          ...
          if (appointmentResult.status === 200) {
            ...
            store.dispatch(
              updateAppointments({
                appointments: appointmentResult.data.appointments,
                totalAppointments: appointmentResult.data.totalAppointments,
              }),
            );
          } else {
          ...
          if (result.status === 200) {
            store.dispatch(
              cacheDoctorCertificateAndLicence({
                licenceDetails: result.data.licenseDetails,
                certificationDetails: result.data.certificationDetails,
              }),
            );
        ...
        if (appointments) {
          ...
          appointments.forEach((appointment) => {
            const isValid = validateDate(
              appointment.appoinmentTime,
              appointment.appoinmentDate,
            );

            if (isValid) {
              dispatch(
                updateNewAppointmentNotification({
                  isNewAppointmentNotification: true,
                  totalAppointments: e.totalAppointmentsForToday,
                }),
              );
            }
          });
      ...
      socket.on(
        ...
        (data: {
          ...
          getStream().then(() => {
            ...
            store.dispatch(
              updateUserCallingData({
                remoteUserId: data.caller._id,
                remoteCallerDetails: data.userCallingDetails,
              }),
            );
      ...
      socket.on("diagnosis", (data: any) => {
        dispatch(
          cacheDiagnosis({ diagnosisChat: { messages: data.messages } }),
        );
      });
  ...
  useEffect(() => {
    ...
    if (
      ...
      store.getState().user.isAunthenticated &&
      localPeerConnection &&
      ...
      remotePeerConnection.iceConnectionState !== "closed" &&
      path !== undefined
    ) {
      console.log("active");
      ...
      try {
        localPeerConnection.addEventListener(
          "icecandidate",
          async (e: RTCPeerConnectionIceEvent) => {
          ...
          userSocket.socket.on(
            ...
            (data: {
              ...
              if (
                ...
                remotePeerConnection.remoteDescription &&
                remotePeerConnection.signalingState !== "closed"
              ) {
                console.log("called");
                try {
                  remotePeerConnection.addIceCandidate(
                    data.user_calling_ice_candidate,
                  );
                } catch (error) {}
              }
            },
              ...
              if (
                ...
                try {
                  localPeerConnection.addIceCandidate(
                    data.user_recieving_call_ice_candidate,
                  );
                } catch (error) {}
              }
            },
        ...
        remotePeerConnection.addEventListener("track", async (e) => {
          ...
          dispatch(
            updateRemoteStream({ remoteStream: e.streams[0], socket: null }),
          );
        });
        ...
        localPeerConnection.onconnectionstatechange = async () => {
          ...
          if (localPeerConnection.connectionState === "connected") {
            ...
            store
              .getState()
              .socket.socket?.emit("failed_to_connect", {
                userId: store.getState().socket.remoteUserId,
              });
          }
        };
            ...
            async (data: {
              ...
              if (
                ...
                try {
                  ...
                  dispatch(
                    updateOfferOrAnswer({ offerReceived: true, socket: null }),
                  );
        ...
        if (userSocket.connected && userSocket.socket) {
          userSocket.socket.on(
            ...
            async (data: {
              ...
              const localPeerConnectionInstance =
                userSocket.localPeerConnectionInstance;
              const remotePeerConnectionInstance =
                userSocket.remotePeerConnectionInstance;
              ...
              if (
                ...
                try {
                  ...
                  await localPeerConnectionInstance?.setRemoteDescription(
                    data.user_receiving_call_answer,
                  );
                  store.dispatch(
                    updatePeerConnectionInstance({
                      localPeerConnectionInstance: localPeerConnectionInstance,
                      remotePeerConnectionInstance:
                        remotePeerConnectionInstance,
                      socket: null,
                    }),
                  );
        ...
        if (userSocket.connected && userSocket.socket) {
          userSocket.socket.on("on_connected", () => {
            store.dispatch(
              updateRemoteConnection({ remoteConnected: true, socket: null }),
            );
          });
        }
      } catch (error: any) {
    ...
    (async () => {
      ...
      if (
        ...
        if (
          localPeerConnection &&
          localPeerConnection.signalingState !== "closed" &&
          localPeerConnection.signalingState === "stable"
        ) {
          console.log("called ");
  ...
  return (
    <div className="w-full h-full relative">
      ...
      {isNewCall && (
        <NewCallUIPage
          userCallingDetails={userCallingDetails!!}
          newCall={userSocket.newInComingCall!!}
          onDecline={() => {
      ...
      )}
      {
        ...
        store.getState().socket.isNewAppointmentNotification && (
          <div
            className={`  cursor-default  w-full h-fit flex  absolute bg-transparent   top-0  z-[1000] md:p-8 p-2    justify-center`}
          >
            <div className="w-full bg-white md:h-[160px] h-[180px] rounded-md shadow-black shadow-sm">
              <div className="md:m-2 relative font-poppins">
                ...
                <div className="md:absolute top-0 right-2 flex gap-4">
                  ...
                  <p className="text-sm">
                    {" "}
                    {store.getState().socket.totalAppointments ?? 0}{" "}
                    Appointments
                  </p>
                </div>
                ...
                <div className="mt-3 w-full  md:h-[110px] p-2 relative">
                  ...
                  <p className="text-sm mt-4">
                    {
                      "it's time to start your consultation as your schedule is currenntly due to avoid refund."
                    }
                  </p>

                  <p
                    className="absolute bottom-0 right-2 flex gap-4 bg-cosmic-primary-color p-1  rounded-md text-white "
                    onClick={() => {
                      dispatch(
                        updateNewAppointmentNotification({
                  ...
                  <p
                    className="absolute  bottom-0 right-20 flex gap-4 bg-red-600  p-1  rounded-md text-white "
                    onClick={() => {
                      dispatch(
                        updateNewAppointmentNotification({
                          isNewAppointmentNotification: false,
