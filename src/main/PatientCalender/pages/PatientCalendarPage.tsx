...
const PatientCalendarPage = () => {
  ...
  return (
    <div
      className={` mt-10 font-poppins w-full h-fit flex flex-col justify-center z-[100px]`}
    >
              ...
              new Array(daysInMonth).fill(0).map(
                (_, index) =>
                  (index === 0 && (
                    <p
                      key={index}
                      className={`${selectedDay === index + 1 && " border-cosmic-color-lightBlue border"}    ${
                        custAppointments &&
                        custAppointments.length > 0 &&
                        custAppointments.find((appointment, _) => {
                          return (
                            appointment
                              .month(appointment.month())
                              .format("MMMM")
                              .toString() === currentMonth &&
                            Number(appointment.format("D")) === index + 1
                          );
                        }) &&
                        " border-cosmic-color-lightBlue border"
                      }         xs:w-[32px] cursor-default hover:border hover:border-cosmic-color-lightBlue w-[60px] p-1 md:m-1 text-center rounded-md xs:text-[10px]`}
                  ...
                  (index + 1 === selectedDay && (
                    <p
                      ...
                      className={`${
                        ...
                        " border-cosmic-color-lightBlue border"
                      }  xs:w-[32px] cursor-default hover:border hover:border-cosmic-color-lightBlue border border-cosmic-color-lightBlue w-[60px] p-1 md:m-1 text-center rounded-md xs:text-[10px]`}
                      onClick={() => setSelectedDay(index + 1)}
                    >
                      {index + 1}
                    </p>
                    ...
                    <p
                      ...
                      className={` ${
                        ...
                        " bg-cosmic-primary-color text-white"
                      }       xs:w-[32px] cursor-default hover:border hover:border-cosmic-color-lightBlue w-[48px] p-1 md:m-1 text-center rounded-md xs:text-[10px]`}
                      onClick={() => setSelectedDay(index + 1)}
                    >
                      {index + 1}
                    </p>
        ...
        <div className="m-4">
          ...
          <div className="w-full m-2 p-3 flex gap-2 relative bg-white rounded-md shadow">
            ...
            <div className="w-full flex flex-col gap-1 relative">
              <p className="w-full text-[14px] md:text-[16px] font-bold">
                {"Grace has an Appointment"}
              </p>

              <p className="text-justify mt-2 text-[14px] md:text-[16px] font-extralight">
                {date?.concat(",").concat(time!!)}
              </p>
            </div>
          </div>
        </div>
