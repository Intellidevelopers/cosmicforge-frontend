...
export  const getDoctorsBySpecificDeparment = async (token:string,department:string) => { 


    const response = await fetch(`${import.meta.env.VITE_BASE_REST_URL}/medics/doctor/department/all`,{
        method:'post',
       headers:{
        "Authorization":`Bearer ${token}`,
        "Content-Type":'application/json'
