...
const DoctorCalendar = () => {
  ...
  const [dateDetails, setDateDetails] = useState<DateDetailsProps>({
  ...
  useEffect(() => {
    const date = dayjs(
      `${dateDetails.selectedYear}-${dayjs().month(dateDetails.currentMonth).format("MM")}`,
      "YYYY-MM",
    ).date(1);
    const firstdayOfTheMonth = date.day() + 1;
    const numberOfDaysInLastMonth = dayjs()
      .month(dateDetails.currentMonth - 1)
      .daysInMonth();
    ...
    lastMonthD = lastMonthD.slice(
      numberOfDaysInLastMonth - firstdayOfTheMonth + 1,
    );
    let currentMonth = [];
    ...
    setDateDetails({
      ...dateDetails,
      numberOfDaysInAmonth: dayjs()
        .month(dateDetails.currentMonth)
        .daysInMonth(),
      firstdayOfTheMonth: firstdayOfTheMonth,
      numberOfDaysInLastMonth: dayjs()
        .month(dateDetails.currentMonth - 1)
        .daysInMonth(),
        ...
        <div className="w-full flex h-full">
          <div className="w-[50px] ms-4 h-full  ">
            ...
            <div className="w-full flex flex-col  place-items-center justify-evenly gap-3 cursor-default">
              {monthsName.map((month, i) => (
                <p
                  className={`${dateDetails.currentMonth === i ? "text-cosmic-primary-color" : "text-cosmic-light-color-call"}`}
                  key={i}
                  onClick={() => {
                    setDateDetails({
                      ...dateDetails,
          ...
          <div className="grid grid-cols-7  w-full h-fit gap-2">
            <div className=" ms-4 col-span-7  grid grid-cols-7">
              {daysName.map((day, i) => (
                <p
                  className={`text-cosmic-primary-color  md:w-[100%] text-center`}
                  key={i}
                >
                  {day}
                </p>
            ...
            <div className=" ms-4 col-span-7  grid grid-cols-7 h-fit">
              {dateDetails.totalCalendarDisplay &&
                dateDetails.totalCalendarDisplay.length > 0 &&
                dateDetails.totalCalendarDisplay.map((number, i) => (
                  <p
                    className={` ${dateDetails.currentDayIndex === i && "bg-cosmic-primary-color text-white"} text-cosmic-primary-color md:w-[100%] h-[70px] font-bold text-[24px] border text-center pt-6`}
                    key={i}
                  >
                    {number}
                  </p>
      ...
      <div className=" w-full h-[300px]  col-span-3 md:col-span-1">
        <div className="w-full p-6 ">
          <div className="w-full h-fit flex gap-4">
            <p className="text-cosmic-primary-color font-bold text-[24px]">
              {dayjs().month(dateDetails.currentMonth).format("MMMM")}
            </p>
