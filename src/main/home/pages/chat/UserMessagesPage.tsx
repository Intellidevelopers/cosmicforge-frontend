import UserMessagesCard, {
  UserMessagesCardProps,
} from "../../component/chat/UserMessagesCard";
...
const UserMessagesPage = () => {
  ...
  const [messagesUpdate, setMessagesUpdate] = useState<
    UserMessagesCardProps[] | null
  >(() => {
    return null;
  });

  const [messagesUpdateCache, setMessagesUpdateCache] = useState<
    UserMessagesCardProps[] | null
  >(() => {
    return null;
  });
  ...
  useMemo(() => {
    if (searchText && searchText.query && messagesUpdateCache) {
      const filteredList = messagesUpdateCache.filter((message) => {
        return new RegExp(`^${searchText.query.toLocaleLowerCase()}`).test(
          message.doctorName?.toLocaleLowerCase()!!,
        );
      });
