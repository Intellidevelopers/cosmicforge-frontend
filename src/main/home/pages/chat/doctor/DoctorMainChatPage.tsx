...
const DoctorMainChatPage = () => {
  const { audioData, isRecording, startRecording, stopRecording } =
    useGetAudioRecorder();
  ...
  const chatPageForWebRef: MutableRefObject<HTMLDivElement | null> =
    useRef(null);
  ...
  return (
    <div className="w-full h-[100vh] overflow-hidden font-poppins">
      ...
      <div className="  w-full grid grid-cols-3 h-full   ">
        <div className="bg-[#272EA7] col-span-3 md:col-span-1 p-6">
          ...
          <div className="mt-6 p-2 flex place-items-center ps-6 bg-cosmic-color-white-light rounded-md  cursor-default">
            ...
            <input
              ...
              className="ms-2 w-full h-full outline-none bg-transparent font-light"
            />
          </div>

          <div
            className={`  w-full  ${chatPageForWebRef.current?.checkVisibility() && " p-5 "} mt-6  overflow-y-auto `}
          >
        ...
        <div
          ref={chatPageForWebRef}
          className="bg-cosmic-bg-chat-background hidden  md:flex flex-col col-span-2"
        >
          <div
            className={`w-full relative h-full ${chatSelected ? "block" : "hidden"} `}
          >
            <div className="bg-white w-full h-[60px] p-3">
              <div className="grid grid-cols-3  ">
                <div className="flex place-items-center gap-2  col-span-2  relative">
                  <img
                    src={chatSelected?.doctorImage}
                    alt="profile-icon"
                      ...
                      onClick={() => {
                        if (userSocket.sessionID) {
                          userSocket.socket?.emit("appointmentSessionEnded", {
                            doctorID: user.data?._id,
                            patientID: userDetails?.details.patientId,
                            endTime: Date.now(),
                            sessionID: userSocket.sessionID,
                          });
                          navigate(-1);
                        }
                      }}
                ...
                {subscription?.planName !== "Free" && (
                  <div
                    className={` ${userSocket.appointmentSessionStarted ? "flex" : "hidden"} w-full  justify-end  gap-3`}
                  >
                    <img
                      className="bg-cosmic-color-white-light rounded-full p-1 w-[30px] h-[30px]"
                      alt="voice-call"
                      src={callIcon}
                      onClick={() => {
            ...
            <div
              ...
              className="bg-transparent h-[73vh] overflow-y-auto"
            >
              {messages?.length &&
                messages?.length > 0 &&
                messages.map((data, i) => (
                  <DoctorChatMessage
                    ...
                    message={data.message}
                    messageType={data.messageType}
                    profilePicture={chatSelected?.doctorImage!!}
            ...
            <div
              className={`${userSocket.appointmentSessionStarted ? "block" : "hidden"}  bg-white w-full h-[90px]`}
            >
              <div className="grid grid-cols-3 ">
                <div
                  className="flex place-items-center   gap-3 col-span-2
"
                >
                  ...
                  <textarea
                    ...
                    value={typedMessage}
                    className=" w-full outline-none resize-none h-[60px] p-2 overflow-y-auto mt-3  "
                    onChange={(e) => {
