...
const DoctorHomeBody = () => {
  ...
  const [
    patientWhoBookedAppopintmentDetails,
    ...
    <div className="w-full h-full  overflow-y-auto  ">
      ...
      <div className="  w-full h-dvh  ">
        ...
        <div className="p-2 min-h-[400px]  overflow-x-hidden overflow-y-hidden  ">
          <div
            className={` ${appointments.appointments && appointments.totalAppointments > 0 ? "hidden" : "block"} w-full h-full flex justify-center place-items-center`}
          >
            <p>No appointment yet</p>
          </div>
          {appointments.appointments && appointments.totalAppointments > 0 && (
            <DoctorTable
              onAppointmentClicked={(data: any, appointmentId: string) => {
      ...
      <div
        className={`${toggleStartSesion ? "block" : "hidden"}  z-[1000] absolute bottom-0 w-full p-3  flex bg-white bg-opacity-80`}
      >
        <p className="font-bold max-w-[50%]  mt-2">
          You are about to start this appoinment session click continue to start
        </p>

        <div className="absolute bottom-0 w-full right-8   flex justify-end place-items-end p-2 gap-5 cursor-default">
          <p
            className="bg-red-600 p-2 text-white rounded-md mt-8 "
            onClick={() => {
          ...
          <p
            className="bg-cosmic-primary-color p-2 text-white rounded-md"
            onClick={() => {
              if (appointmentIcon && patientWhoBookedAppopintmentDetails) {
                dispatch(
                  updateAppointmentSession({ appointmentSessionStarted: true }),
                );

                userSocket.socket?.emit("appointmentSessionStarted", {
                  doctorID: user.data?._id,
                  patientID:
                    patientWhoBookedAppopintmentDetails?.details.patientId,
                  startTime: Date.now(),
                  caller: "doctor",
                  appointmentId,
                });
