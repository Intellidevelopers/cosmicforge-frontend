...
const DoctorChartGraph = () => {
  ...
  const appointmentDetails = useSelector(
    (state: RootReducer) => state.appointments,
  );
  ...
  useMemo(() => {
    let monthd = [
      { month: "Jan", numberOfAppointment: 0 },
      { month: "Feb", numberOfAppointment: 0 },
      { month: "Mar", numberOfAppointment: 0 },
      { month: "Apr", numberOfAppointment: 0 },
      { month: "May", numberOfAppointment: 0 },
      { month: "Jun", numberOfAppointment: 0 },
      { month: "July", numberOfAppointment: 0 },
      { month: "Aug", numberOfAppointment: 0 },
      { month: "Sept", numberOfAppointment: 0 },
      { month: "Oct", numberOfAppointment: 0 },
      { month: "Nov", numberOfAppointment: 0 },
      { month: "Dec", numberOfAppointment: 0 },
    ];
    ...
    if (
      ...
      appointmentDetails.totalAppointments > 0 &&
      monthNumber.length < 12
    ) {
      monthd.forEach((monthDetail) => {
        const count = appointmentDetails.appointments?.filter((appointment) => {
          return (
            appointment.appointmentDate.includes(monthDetail.month) &&
            appointment.patientDetails.vitalSigns?.gender === "male"
          );
        });
        ...
        if (count) {
          ...
          if (
            appointmentDetails.totalAppointments > 10 &&
            appointmentDetails.totalAppointments < 50
          ) {
          ...
          if (
            appointmentDetails.totalAppointments >= 50 &&
            appointmentDetails.totalAppointments < 100
          ) {
            monthNumber.push(normalizeValue(count.length, 500));
            return;
          }
          ...
          if (
            appointmentDetails.totalAppointments >= 100 &&
            appointmentDetails.totalAppointments < 500
          ) {
        ...
        const countForFemale = appointmentDetails.appointments?.filter(
          (appointment) => {
            return (
              appointment.appointmentDate.includes(monthDetail.month) &&
              appointment.patientDetails.vitalSigns?.gender === "female"
            );
          },
        );
  ...
  return (
    <div className=" w-full  h-[400px]  bg-white rounded-md">
      <div className="w-full   p-4 flex  place-items-center gap-2">
        <p className="font-bold text-[16px] text-left min-w-[40%]">
          Patient Appointment
        </p>
