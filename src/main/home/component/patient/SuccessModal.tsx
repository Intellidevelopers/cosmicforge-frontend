...
const PaymentSuccessCard = ({ onClose }: PaymentSuccessProps) => {
  ...
  return (
    <div className="bg-white rounded-md shadow-lg p-16 max-w-2xl w-full h-[60%] text-center relative">
      <i
        className="fa fa-times absolute text-xl right-5 hover:text-red-600 top-2"
        onClick={() => {
          onClose();
        }}
      />
      ...
      <button
        className="bg-[#272EA7] hover:bg-indigo-700 text-white font-medium py-2 w-[50%] px-6 rounded-md transition"
        onClick={() => {
