  ...
  useEffect(() => {
    if (
      ...
      state.title.trim().toLowerCase() !==
        "General Medicine".trim().toLowerCase()
    ) {
      setLoading(false);
      setDoctorDetails([]);
    ...
    (async () => {
      ...
      try {
        const result = await getDoctorsBySpecificDeparment(
          user.data?.token!!,
          state?.title,
        );
        setLoading(false);
    ...
    <div className="font-poppins w-full   relative  h-dvh overflow-x-hidden    overflow-y-hidden flex flex-col cursor-default">
        ...
        onSearchFired={(path, query) => {
          if (path === state.title && query) {
            const filter = doctorDetailsCache.filter((doctorDetails) => {
              return (
                new RegExp(`^${query.toLocaleLowerCase()}`).test(
                  doctorDetails.userId.fullName?.toLocaleLowerCase()!!,
                ) ||
                doctorDetails.userCosmicID === query ||
                new RegExp(`^${query.toLocaleLowerCase()}`).test(
                  doctorDetails.userId.lastName?.toLocaleLowerCase()!!,
                )
              );
            });
      ...
      <div className="w-full  ">
        ...
        <div className="w-full flex justify-evenly gap-3  text-cosmic-primary-color mt-5 p-2">
          <p
            className={`${activState.all && "bg-cosmic-primary-color text-white "}  md:w-[40%] w-[50%] p-2 flex justify-center place-items-center  text-center  rounded-md border border-cosmic-color-border-color hover:bg-cosmic-primary-color hover:text-white`}
            onClick={() => {
              setActiveState({
                ...activState,
                all: true,
          ...
          <p
            className={`${activState.topRatings && "bg-cosmic-primary-color text-white "}  p-2  md:w-[40%] w-[50%] flex flex:1 justify-center place-items-center   rounded-md border border-cosmic-color-border-color hover:bg-cosmic-primary-color hover:text-white`}
            onClick={() => {
              setActiveState({
                ...activState,
                all: false,
        ...
        <div
          className={`${loading ? "flex" : "hidden"}  w-full h-dvh  justify-center mt-[20%]`}
        >
          <Loader />
        </div>

        <div
          className={`${!loading && doctorDetails.length === 0 ? "flex" : "hidden"}  w-full h-dvh  justify-center mt-[20%]`}
        >
          <p>{customMessage}</p>
        </div>
        <div
          className={`${!loading ? "flex" : "hidden"} w-full h-[80vh]  p-3  pb-[100px]  flex-col gap-3 overflow-y-auto `}
        >
