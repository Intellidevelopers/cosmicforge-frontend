...
const VirtualConsultBody = () => {
  ...
  const localVideoStream: MutableRefObject<HTMLVideoElement | null> =
    useRef(null);
  const localAudioStream: MutableRefObject<HTMLAudioElement | null> =
    useRef(null);
  ...
  const [callState, setCallState] = useState<
    "calling" | "connected" | "failed to connect" | "call ended" | null
  >("calling");
      ...
      if (
        ...
        (async () => {
          try {
            ...
            dispatch(
              updateUserCallingData({ remoteUserId: data.docId, socket: null }),
            );
          } catch (error) {
            console.log(error);
          }
  ...
  useEffect(() => {
    if (
      ...
      socketCon.socket?.emit("appointmentSessionStarted", {
        doctorID: socketCon.remoteUserId,
      ...
      if (socketCon.socket) {
        socketCon.socket.on("sessionID", (data: { sessionID: string }) => {
          ...
          store.dispatch(
            updateAppointmentSession({ sessionID: data.sessionID }),
          );
        });
      }
      ...
      <div className="w-full h-full ">
        ...
        <div className="w-full ps-0  mt-4">
          <div className="hidden m-8 md:flex place-items-center gap-1">
            <i
              className="fa fa-chevron-left fa-2xl"
              aria-hidden="true"
              onClick={() => {
                dispatch(tearDownConnection({ tearDown: true, socket: null }));
                cancelMediaStream();
          ...
          <div className="w-full relative ">
            <div
              className=" h-[100px]  w-full  flex flex-col justify-center place-items-center text-white font-bold absolute z-[200] 
"
            >
              <div
                className={`${requestingForModeChangeRemote ? "flex bg-black" : "hidden"}  w-full cursor-default  justify-center gap-2 flex-col place-items-center`}
              >
                <p className="text-white text-[12px]" onClick={() => {}}>
                  {`${socketCon.remoteCallerDetails?.name ?? data?.doctorName} is requesting to switch to ${tempModeOfCall}`}{" "}
                </p>
            ...
            <div className="w-full   flex justify-center place-items-center absolute  mt-6 z-50  font-light ">
              <div
                className={`w-full ${socketCon.callMode === "video" && "text-white"} flex flex-col justify-center place-items-center gap-2`}
              >
                <p>
                  Dr {data?.doctorName ?? socketCon.remoteCallerDetails?.name}
                </p>
                <p>{data?.department ?? ""}</p>
            ...
            <div className="md:ps-10 md:pe-10 h-[450px] ">
              <div
                className={`w-full  ${socketCon.callMode === "video" ? "block" : "hidden"}`}
              >
                <video
                  ref={remoteVideoSteam}
                  autoPlay
                  className="h-[450px] w-full  bg-black  opacity-90  object-cover"
                />
              ...
              <div
                className={`h-full  w-full  ${socketCon.callMode === "audio" ? "block" : "hidden"}`}
              >
                <div className="w-full h-full flex justify-center place-items-center ">
                  <div className="w-[200px] h-[200px] rounded-full object-contain mt-16 ">
                    <img
                      ...
                      src={
                        data?.doctorImage ??
                        socketCon.remoteCallerDetails?.profilePicture
                      }
                      alt="doctors profile"
                    />
          ...
          <div className="w-full flex flex-col place-items-center justify-center p-1 gap-2 mt-2">
            <p className="bg-cosmic-light-color-call w-[100px] text-center text-white font-light p-1 rounded-md">
              {counter}
            </p>
            ...
            <div className="bg-cosmic-light-color-call  flex p-2 gap-2">
              ...
              <div
                ...
                onClick={async () => {
                  ...
                  console.log("fired...");
                  dispatch(
                    updateCallInitialization({ isCallInitiated: false }),
                  );
                  dispatch(
