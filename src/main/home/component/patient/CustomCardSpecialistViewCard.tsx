...
const CustomCardSpecialistViewCard = ({
  details,
}: CustomCardSpecialistViewCardProps) => {
  ...
  return (
    <div
      className="bg-white w-full  h-[150px] flex  relative  "
      onClick={() => {
    ...
    >
      <div className=" w-[200px] h-[150px]  relative border border-cosmic-color-border-color rounded-md">
         ...
         */}
        <img
          className="bg-black bg-opacity-30 object-cover h-full w-full "
      ...
      <div className="w-full flex flex-col gap-4 md:ms-4 relative">
        <div className="flex  place-items-center mt-2">
          <p className="font-bold text-[14px]  ">
            Dr {details?.userId?.fullName ?? details?.userId?.lastName}
          </p>
        ...
        <div className="absolute mt-2 right-2  text-[14px]  flex md:ustify-center  place-items-center gap-2">
           ...
           *   <p className='text-cosmic-silver-color '>{/**rrating }</p>
           */}
        </div>
