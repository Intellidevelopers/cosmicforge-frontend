                ...
                <div className="grid grid-cols-7">
                    ...
                    <div className="col-span-7 grid grid-cols-7 gap-3 mt-4">
                        {daysInMonth && new Array(daysInMonth).fill(0).map((_, index) => (
                            (index === 0) && <p key={index} className="xs:w-[32px] cursor-default hover:border hover:border-cosmic-color-lightBlue w-[50px] p-1 md:m-1 text-center rounded-md xs:text-[10px]" style={{ gridColumn: firstDay }} onClick={() => setSelectedDay(index + 1)}>{index + 1}</p> ||
