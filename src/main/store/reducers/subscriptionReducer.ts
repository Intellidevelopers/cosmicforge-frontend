...
const subscriptionSlice = createSlice({
  ...
  reducers: {
    cacheSubscription(state, action: PayloadAction<SubscriptionProps>) {
      state.userSubcription =
        action.payload.userSubcription ?? state.userSubcription;
    },

    updateCurrencyMode(state, action: PayloadAction<SubscriptionProps>) {
      state.currentCurrencyMode =
        action.payload.currentCurrencyMode ?? state.currentCurrencyMode;
    },
  },
});

export const { cacheSubscription, updateCurrencyMode } =
