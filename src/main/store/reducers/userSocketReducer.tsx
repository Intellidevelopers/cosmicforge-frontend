...
const userSocketSlice = createSlice({
  ...
  reducers: {
    ...
    updateUserCallingData(state, action: PayloadAction<UserSocketProps>) {
      ...
      state.remoteCallerDetails =
        action.payload.remoteCallerDetails ?? state.remoteCallerDetails;
    },
    ...
    updateRemoteDescription(state, action: PayloadAction<UserSocketProps>) {
      state.remoteDescription =
        action.payload.remoteDescription ?? state.remoteDescription;
    },

    updateLocalDescription(state, action: PayloadAction<UserSocketProps>) {
      state.localDescription =
        action.payload.localDescription ?? state.localDescription;
    },

    updatePeerConnectionInstance(
      state,
      action: PayloadAction<UserSocketProps>,
    ) {
      state.localPeerConnectionInstance =
        action.payload.localPeerConnectionInstance ?? null;
      state.remotePeerConnectionInstance =
        action.payload.remotePeerConnectionInstance ?? null;
    },
    ...
    updateRingTone(state, action: PayloadAction<UserSocketProps>) {
      state.startPlayingRingTone =
        action.payload.startPlayingRingTone ?? state.startPlayingRingTone;
    },

    updateRemoteConnection(state, action: PayloadAction<UserSocketProps>) {
      state.remoteConnected =
        action.payload.remoteConnected ?? state.remoteConnected;
    },
    ...
    updatePeerNewConnectionInstance(
      state,
      action: PayloadAction<UserSocketProps>,
    ) {
      state.newPeerConnectionInstance =
        action.payload.newPeerConnectionInstance ??
        state.newPeerConnectionInstance;
    },

    updateCallInitialization(state, action: PayloadAction<UserSocketProps>) {
      state.isCallInitiated =
        action.payload.isCallInitiated ?? state.isCallInitiated;
    },

    updateIncomingCall(state, action: PayloadAction<UserSocketProps>) {
      state.newInComingCall =
        action.payload.newInComingCall ?? state.newInComingCall;
    },
    ...
    updateLocalConection(state, action: PayloadAction<UserSocketProps>) {
      state.locallyConnected =
        action.payload.locallyConnected ?? state.locallyConnected;
    },

    updateCallAnswered(state, action: PayloadAction<UserSocketProps>) {
      state.onCallAnswered =
        action.payload.onCallAnswered ?? state.onCallAnswered;
    },
    ...
    updateNewAppointmentNotification(
      ...
      state.isNewAppointmentNotification =
        action.payload.isNewAppointmentNotification ??
        state.isNewAppointmentNotification;
      state.totalAppointments =
        action.payload.totalAppointments ?? state.totalAppointments;
    },

    updateAppointmentSession(state, action: PayloadAction<UserSocketProps>) {
      state.appointmentSessionStarted =
        action.payload.appointmentSessionStarted ??
        state.appointmentSessionStarted;
    ...
    updateNavigationHistory(state, action: PayloadAction<UserSocketProps>) {
      state.doctorNavHistory =
        action.payload.doctorNavHistory ?? state.doctorNavHistory;
      state.patientNavHistory =
        action.payload.patientNavHistory ?? state.patientNavHistory;
    },
  },
});
