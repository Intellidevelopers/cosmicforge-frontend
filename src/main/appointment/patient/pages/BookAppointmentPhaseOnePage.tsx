...
const BookAppointmentPhaseOnePage = () => {
    ...
    const [appointmentmentDetails, setAppointmentmentDetails] = useState<{
        date: string;
        time: string;
        appointmentType: "None" | "Virtual" | "In-Person";
    ...
    const [doctorsAppointmentDetails, setDoctorsAppointmentDetails] = useState<
        any[]
    >([]);
    ...
    useEffect(() => {
        ...
        if (
            doctorsAppointmentDetails.length > 0 &&
            time &&
            time.split("-").length >= 1 &&
            ...
            if (appointmentmentDetails.date) {
                selectedDay = appointmentmentDetails.date
                    .split(" ")[1]
                    .replace(/[a-z]/g, "");
            ...
            if (filteredData.length > 0) {
                ...
                while (currentTime <= endFormattedTime) {
                    ...
                    const seletedDateDay = appointmentmentDetails.date.split(' ')[1].replace(/[a-z]/g, '')

                    const todayDateDay = recentTime.toLocaleDateString('en-US', {
                        day: 'numeric'
                    })
                    ...
                    if (seletedDateDay === todayDateDay && recentTime.getTime() <= currentTime.getTime()) {
                        availableTimeCache.push(timeToAdd);
                        availableTime.push(timeToAdd);
                    }

                    if (Number(seletedDateDay) > Number(todayDateDay)) {
                        availableTimeCache.push(timeToAdd);
                        availableTime.push(timeToAdd);
                    }
                ...
                const filteredTimeData = availableTime.filter((timeToAdd) => {
                    ...
                    const t = filteredData.find((details) => {
                        if (
                            (details.appointmentTime.split("-")[0].trim() ===
                                timeToAdd.trim() ||
                                details.appointmentTime.split("-")[1] === timeToAdd.trim()) &&
                            details.medicalPersonelID._id === state?.doctorId
                        ) {
                            return true;
                        } else {
                            return false;
            ...
            {
                /**
                 * If any appoinment not booked at all it should  generate time according to doctor's provided time.
                 */
            }
            ...
            if (
                ...
                time.split("-").length >= 1 &&
                time.split("-")[0].match(/[0-9]/g) &&
                time.split("-")[1].match(/[0-9]/g) && appointmentmentDetails.date
            ) {
                let availableTime: string[] = [];
    ...
    return (
        <div
            ref={bodyRef}
            className="relative bg-[#F5F5F5] bg-opacity-50  cursor-default  font-poppins w-full p-5 overflow-x-hidden  h-full mb-20 overflow-y-auto"
        >
            <div
  ...
  "
            ...
            <div className="w-full p-3 ">
                <div className="w-full">
                    ...
                    <ul className="flex flex-wrap gap-2 justify-center ">
                        {timeSelected &&
                            ...
                            availableTimeList.map((d, k) => (
                                <li
                                    key={k}
                                    className={` border  ${timeSelected?.trim() === d.trim() ? "border-cosmic-primary-color  " : "border-transparent"}  w-[100px] m-4 h-[90%] p-2 hover:border-cosmic-primary-color `}
                                    onClick={() => {
                                        setTimeSelected(d);
                                        ...
                                        if (
                                            ...
                                            t.setHours(
                                                Number(d.split(":")[0]) === 12
                                                    ...
                                                    : Number(d.split(":")[0]),
                                                Number(
                                                    d.split(":")[1].replace(/[a-z A-Z]/g, "") === "00"
                                                        ? 0
                                                        : d.split(":")[1].replace(/[a-z A-Z]/g, ""),
                                                ),
                                                0,
                                                0,
                                            );
                ...
                <h5 className=" text-center  text-cosmic-primary-color mt-8 ">
                    Duration
                </h5>
                    ...
                    onChange={(e) => {
                        ...
                        time.setHours(
                            timeSelected
                                ...
                                .trim()
                                .toLowerCase() === "pm" &&
                                Number(timeSelected.split(":")[0]) !== 12
                                ? timeUtc.get(Number(timeSelected?.split(":")[0]))!!
                                : Number(timeSelected?.split(":")[0]),
                            Number(
                                timeSelected?.split(":")[1].replace(/[a-z A-Z]/g, "") === "00"
                                    ? 0
                                    : timeSelected?.split(":")[1].replace(/[a-z A-Z]/g, ""),
                            ),
                            0,
                            0,
                        );
                        ...
                        time.setMinutes(
                            time.getMinutes() + (e.target.value.includes("30") ? 30 : 15),
                        );

                        setAppointmentmentDetails({
                            ...appointmentmentDetails,
                            time: timeSelected?.concat("-").concat(
                                new Date(time).toLocaleTimeString("en-US", {
                                    timeStyle: "short",
                                }),
                            )!!,
            ...
            */}
            ...
            <div className="w-full p-8 flex flex-col place-items-center justify-center">
                <p
                    className="bg-cosmic-primary-color p-2 text-center text-white rounded-md w-[300px]"
                    onClick={() => {
                        setErrorMesage("");

                        const dataComplete = Object.entries(appointmentmentDetails).every(
                            ([, value]) => {
                        ...
                        if (!isValid) {
                            ...
                            setErrorMesage(
                                "please reshedule the time as doctor will not be available at that time.",
                            );
                        ...
                        if (
                            !state.pricingForThirtyMins &&
                            appointmentmentDetails.sessionDuration === 30
                        ) {
