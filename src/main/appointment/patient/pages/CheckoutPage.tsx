...
const CheckoutPage = () => {
    ...
    const total = (state?.appointmentmentDetails.sessionDuration === 15) ? Number(state?.pricing.replace(",", "")) + 1000 : Number(state?.pricingForThirtyMins.replace(",", "")) + 1000;

    const [paymentSuccessful, setPaymentSuccessful] = useState<boolean>();

    return (
        <div className="relative  bg-[#F5F5F5] bg-opacity-50  cursor-default overflow-auto  font-poppins w-full md:p-5 overflow-x-hidden h-full overflow-y-auto pb-8 mb-20">
            <div
                ...
                <div className="w-full flex gap-5 mt-5 bg-white shadow shadow-black  p-1 relative">
                    ...
                    <div className="text-[12px] md:text-[14px]">
                        <p className="font-bold">
                            {state?.appointmentmentDetails.appointmentType} Appointment
                        </p>
                        <p className="font-light mt-2">
                            {state?.udpatedDate ?? state?.appointmentmentDetails.date},{" "}
                            {state?.appointmentmentDetails.time}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="w-full mt-6">
                ...
                <div className="w-full mt-2 border shadow shadow-black bg-white">
                    <div className="w-full relative p-2 flex ">
                        ...
                        <p className="absolute top-2 right-8 decoration-cosmic-primary-color font-bold min-w-[90px] text-end">
                            N {`${new Intl.NumberFormat().format((state?.appointmentmentDetails.sessionDuration === 15) ? Number(state?.pricing.replace(",", "")) : Number(state?.pricingForThirtyMins.replace(",", "")))} `}
                        </p>
                    </div>
                    ...
                    <div className="w-full relative p-2 flex  ">
                        ...
                        <p className="absolute top-2 right-8 decoration-cosmic-primary-color font-bold min-w-[90px] text-end">
                            N {new Intl.NumberFormat().format(1000)}
                        </p>
                    </div>
                    ...
                    <div className="w-full relative p-2 flex  ">
                        ...
                        <p className="absolute top-2 right-8 decoration-cosmic-primary-color font-bold min-w-[90px] text-end">
                            N {new Intl.NumberFormat().format(total)}{" "}
                        </p>
                    </div>
            ...
            <div className="mt-3 bg-cosmic-light-color-call p-6 flex justify-center flex-col place-items-center text-white">
                ...
                <p className=" mt-2 decoration-cosmic-primary-color font-bold">
                    N {`${new Intl.NumberFormat().format(total)} `}
                </p>
            </div>
            ...
            <div className="mb-10 ">
                <PaystackButton
                    ...
                    onSuccess={async (e: { reference: string; status: string }) => {
                        if (e.reference) {
                            await book_appointment(user.data?.token!!, {
                                doctorId: state?.doctorId,
                                date: state?.appointmentmentDetails.date,
                                time: state?.appointmentmentDetails.time,
                                appointmentType: state?.appointmentmentDetails.appointmentType,
                ...
                >
                    <div className="mt-3 flex justify-center flex-col place-items-center">
                        <p className="bg-cosmic-primary-color w-[200px] p-2 m-2 text-center text-white border rounded-md font-light">
                            Continue
                        </p>
                    </div>
                </PaystackButton>
            ...
            {paymentSuccessful && (
                <div
                    className={`  absolute w-screen h-screen top-0 left-0 bg-black bg-opacity-50  flex place-items-center md:ps-[20%]`}
                >
                    <PaymentSuccessCard
                        onClose={() => {
