...
const DoctorBioPage = () => {
    ...
    (async () => {
      ...
      try {
        ...
        if (result.status === 200) {
          ...
          }else if (time && time.split('-').length >= 1 && (time.split('-')[0].match(/[0-9]/g) && time.split('-')[1].match(/[0-9]/g))) {
  ...
  " src={state?.doctorImage ?? '/'} style={{}} />
        ...
        <div className="w-full h-fit md:h-[80px] absolute ps-10 bottom-0 bg-gradient-to-t from-cosmic-doc-gradient-1 to-cosmic-doc-gradient-2  bg-cosmic-light-color-call bg-opacity-90">
          <div className="w-full flex place-items-center gap-3 h-full p-2 text-white justify-between flex-wrap">
            ...
            <div>
              ...
              <p className="font-bold overflow-hidden   text-[14px] md:text-[16px]   min-w-[80px] max-w-[80px] md:max-w-fit   text-nowrap text-ellipsis">{state?.department}</p>
            </div>
    ...
    <div className="ms-3 me-3">
      ...
      <p className="font-extralight text-justify text-pretty leading-8">{state?.bio}</p>
    </div>
