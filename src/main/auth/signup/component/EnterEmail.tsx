...
import { resend_otp, sign_up_user } from "../service/service";
...
const EnterEmail: React.FC<component> = ({
  email,
  setEmail,
  step,
  setStep,
  ...
  return (
    <div
      className={`h-screen ${pageWidth} ${pageOp} bg-gray-100 overflow-hidden transition-all duration-500 ease-in-out md:px-0 px-[5%] flex flex-col justify-center items-center`}
    >
      <Link
      ...
      <div className="md:w-[400px] md:h-fit h-screen md:text-center text-left flex flex-col md:pb-0 pb-[10%] justify-between items-center md:justify-around md:pt-0 pt-[20%]  gap-[15px] w-[100%]">
        ...
        <div className="flex flex-col justify-center place-items-center gap-2 w-[100%] mt-[10%]">
          ...
          <input
            ...
            className="w-[100%] px-[10px]  outline-none rounded-[5px] bg-gray-300 md:h-[48px] h-[50px] md:mt-0 mt-[40px]"
          />
          <span
            className={`text-red-500 text-[14px] ${notValidError ? "opacity-[100%]" : "opacity-[0%]"}`}
          >
            {errorMessage}
          </span>
          <span
            className={` ${errorMessage.includes("Already sent otp") ? "block" : "hidden"}    underline hover:text-blue-600 decoration-blue-700 cursor-default`}
            onClick={() => {
              resendOTP();
              setStep(step + 1);
            }}
        ...
        <button
          ...
          className={`${isLoading ? "bg-[#272EA7]/80" : "bg-[#272EA7]"} h-[48px] w-[100%] md:mt-[80px] hover:bg-[#272EA7]/80 text-white font-bold flex flex-row justify-center items-center rounded-[5px]`}
        >
