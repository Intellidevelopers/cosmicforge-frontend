...
const EnterPersonalDetails: React.FC = () => {
  ...
  const atleast8CharatersRef: MutableRefObject<HTMLDivElement | null> =
    useRef(null);
  ...
  const startReg = async (e: React.KeyboardEvent<HTMLFormElement>) => {
    ...
    if (!agreedToTerms) {
      setErrorMesage(
        "Enure you agree to terms and conditions by checking the box.",
      );
      return;
    }
    ...
    try {
      ...
      if (result.status === 200) {
        ...
        setIsLoading(false);
        dispatch(
          authenticateUser({ isAunthenticated: true, data: result.data }),
        );
  ...
  return (
    <div className=" font-poppins flex flex-row bg-gray-100 py-[50px] justify-center h-screen w-screen">
      <button
        onClick={() => {
          navigate("/account/signup/verify-email");
        }}
        className="flex absolute md:top-[50px] top-[5%] flex-row gap-2 items-center font-bold md:w-[80%] w-[95%] h-fit"
      >
        <IconContainer
          image={backIcon}
          classes="rotate-180"
      ...
      <div className="flex-col items-center mt-[4%] md:mt-0 h-[90%] flex md:w-[400px] w-[90%] md:h-fit">
        ...
        <div className="w-full flex flex-col place-items-center justify-center">
          <span className="text-[23px] md:text-[17px] font-bold  md:self-center mt-2">
            Welcome!
          </span>
          <span className="mt-2 text-[17px] md:text-[16px]  ">
            Start your journey with us.
          </span>
        </div>
        <form
          onSubmit={startReg}
          className="mt-[10px] w-[100%] flex flex-col gap-[10px] md:gap-4"
        >
                ...
                if (e.target.value.length < 8) {
                  if (e.target.value) {
                    if (atleast8CharatersRef.current) {
                      atleast8CharatersRef.current.style.backgroundColor =
                        "red";
                    }
                  } else {
                    if (atleast8CharatersRef.current) {
                ...
                if (e.target.value && /[A-Z]/.test(e.target.value)) {
                  ...
                  if (text && text.length === 1) {
                    if (upperCaseRef.current) {
                      upperCaseRef.current.style.backgroundColor =
                        "rgba(255, 204, 0, 1)";
                    }
                  } else if (upperCaseRef.current) {
                ...
                if (e.target.value && /[a-z]/.test(e.target.value)) {
                  ...
                  if (text && text.length === 1) {
                    if (lowerCaseRef.current) {
                      lowerCaseRef.current.style.backgroundColor =
                        "rgba(255, 204, 0, 1)";
                    }
                  } else if (lowerCaseRef.current) {
                ...
                if (
                  ...
                  if (text && text.length === 1) {
                    if (symbolRef.current) {
                      symbolRef.current.style.backgroundColor =
                        "rgba(255, 204, 0, 1)";
                    }
          ...
          <div className="flex flex-col gap-1">
            <label className="text-[17px] md:text-[16px]">
              Confirm Password
            </label>
            <input
          ...
          <div className="w-full flex  gap-5 mt-4">
            <div
              ref={atleast8CharatersRef}
              className="w-[35px] h-[3px] bg-gray-300 rounded-md"
            ></div>
            <div
              ref={upperCaseRef}
              className="w-[35px] h-[3px] bg-gray-300 rounded-md"
          ...
          <div className="w-full flex flex-col place-items-center justify-center ">
            <p className={`${errorMessage ? "block" : "hidden"} text-red-600`}>
              {errorMessage}
            </p>
          ...
          <div className="flex gap-2 mt-[10px] flex-row items-center">
            ...
            <span className="text-[14px] md:text-[14px] font-light">
              ...
              <Link
                ...
                className="hover:text-[#272EA7]/80 text-[#272EA7]"
              >
                {" "}
                Privacy Policy{" "}
              </Link>
          ...
          <button
            type="submit"
            className=" mt-3  md:h-[40px] h-[45px] md:w-[100%]  md:ml-0 ml-[5%] w-[90%] rounded-[5px] hover:bg-[#272EA7]/70 bg-[#272EA7] cursor-pointer text-white"
          >
            Sign Up
          </button>
        </form>
