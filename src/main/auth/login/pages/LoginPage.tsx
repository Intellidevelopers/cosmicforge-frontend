...
import { sign_up_user_wih_google } from "../../signup/service/service";
...
const LoginPage: React.FC = () => {
  ...
  const [makePasswordInVisible, setMakePasswordInVisible] =
  ...
  return (
    <div className="flex flex-row bg-gray-100 py-[50px] justify-center h-screen w-screen">
      ...
      <div className="flex-col items-center mt-[10%] md:mt-8 h-[90%] flex md:w-[400px] w-[90%] md:h-fit">
        ...
        <span className="text-[23px] font-bold self-start md:self-center mt-2">
          Log In
        </span>
        <span className="mt-2 text-[17px] md:text-[16px] self-start md:self-center">
          Welcome back.
        </span>

        <div
          onSubmit={startReg}
          className="mt-[17px] w-[100%] flex flex-col gap-3 md:gap-2"
        >
          ...
          <div className="flex flex-col gap-1 relative">
            ...
            <input
              ...
              className="w-[100%] px-[10px] outline-none rounded-[5px] bg-gray-300 md:h-[40px] h-[50px]"
              type={makePasswordInVisible ? "password" : "text"}
            />
            {!makePasswordInVisible && (
              <i
                className="fa fa-eye absolute top-[60%] right-3"
                onClick={() => {
                  setMakePasswordInVisible(true);
                }}
              />
            )}
            ...
            {makePasswordInVisible && (
              <i
                className="fa fa-eye-slash absolute top-[60%] right-3"
                onClick={() => {
                  setMakePasswordInVisible(false);
                }}
              />
            )}
          </div>
          ...
          <button
            onClick={startReg}
            type="submit"
            className="md:h-[40px] h-[45px] md:w-[100%] mt-[20px] md:ml-0 ml-[5%] w-[90%] rounded-[5px] hover:bg-[#272EA7]/70 bg-[#272EA7] cursor-pointer text-white"
          >
            Log In
          </button>
        </div>
        ...
        <div className="flex flex-col gap-[15px] mt-[30px] w-[100%] items-center">
          <div className="flex flex-row gap-[20px] w-[100%] justify-center items-center">
            ...
            <button
              className="w-[55px] h-[55px] md:w-[40px] md:h-[40px] hover:scale-[105%] flex flex-col justify-center items-center border-[1px] border-gray-300 rounded-[4px]"
              onClick={async () => {
                try {
                  console.log("firing.........");
                  const token = (
                    await sign_up_user_wih_google({ authType: "login" })
                  ).data;
