...
} from "../../../store/reducers/doctorCertificateAndLicence";
...
import { uploadCertificate } from "../../service";
...
const UploadLiscenceDetails = () => {
  ...
  const certificateDetails = useSelector(
    (state: RootReducer) =>
      state.doctorCertificateAndLicence.uploadCertificationDetails,
  );
  ...
  const [startFaceVerificationProcess, setStartFaceVerificationProcess] =
    useState(false);
  ...
  return (
    <div className="bg-[#F5F5F5] font-poppins relative z-1 h-screen overflow-y-auto pb-4  ">
      ...
      <div
        className=" hidden md:flex  cursor-default  place-items-center gap-3 m-5"
        onClick={() => {
          navigate(-1);
        }}
      >
      ...
      <div className="w-full relative flex justify-center gap-1 p-2">
        {new Array(3)
          ...
          .map((_, index) =>
            index === activeState ? (
              <div className="w-[50%] bg-cosmic-primary-color h-[10px] rounded-r-md rounded-l-md " />
            ) : (
          ...
          onClick={async () => {
            ...
            if (path === "") {
              if (
                ...
                (!certificateDetails.fullName ||
                  !certificateDetails.institution ||
                  !certificateDetails.certificateNo ||
                  !certificateDetails.date ||
                  !certificateDetails.certificate ||
                  !certificateDetails.photoWithCertification)
              ) {
                ...
                .then(({ data: { text } }) => {
                  const isCertificateValid =
                    text
                      ...
                      .toLowerCase()
                      .includes(
                        "Federal republic of nigeria".trim().toLowerCase(),
                      ) ||
                    text
                      .trim()
                      .toLowerCase()
                      .includes(
                        "Medical & Dental Council Of Nigria"
                          .trim()
                          .toLowerCase(),
                      );
                  setProcessing(false);
                  if (isCertificateValid) {
                    navigate("/doctor/certificate-details-upload/id");
                  } else {
                    setErrorMessage("Invalid certificate provided.");
                  }
                })
                .catch(() => setErrorMessage("Invalid certificate provided."));
            ...
            if (path === "id") {
              if (
                ...
                (!certificateDetails.country ||
                  !certificateDetails.docummentType ||
                  !certificateDetails.documentId ||
                  !certificateDetails.documentHoldName ||
                  !certificateDetails.documentImage ||
                  !certificateDetails.pictureWithDocument)
              ) {
                setErrorMessage("Please enter the fields completely.");
                return;
              }
              ...
              Textextractor.recognize(
                ...
                .then(({ data: { text } }) => {
                  ...
                  setProcessing(false);
                  if (isLicenceValid) {
                    navigate("/doctor/certificate-details-upload/face-id");
                  } else {
            ...
            if (path === "face-id") {
              if (
                ...
                setErrorMessage("One or two fields are missing check please.");
                return;
              }
      ...
      <div
        className={` ${startFaceVerificationProcess || showUploadComp ? "flex" : "hidden"} absolute top-[70px] bg-black h-screen w-screen bg-opacity-15  place-items-center justify-center`}
      >
        <div
          className={`${showUploadComp ? "hidden" : "block"} md:w-[500px] min-h-[530px] bg-white p-4 mt-3 `}
        >
          <div className="w-full relative flex place-items-center">
            <p className="font-bold">Face Verification</p>
            <i
              className="fa fa-times absolute right-0"
              onClick={() => {
          ...
          <p className="font-light text-center mt-10">
            Face the camera and hold your device steady.
          </p>

          <div
            className={` w-full  flex-col place-items-center justify-center mt-8 h-fit `}
          >
            <div className="ellipse   ">
              {
                <video
                  ref={videoRef}
                  className={`ellipse border-[2px] ${faceDetected ? "border-green-600" : "border-red-700"}  object-fill h-full w-full bg-black bg-opacity-20`}
                  autoPlay
                />
              }
              ...
              <canvas
                ref={canvasRef}
                className={`${imageCaptured ? "visible" : "invisible"}  ellipse border-[2px]  border-red-700 object-fill h-full w-full bg-black bg-opacity-20`}
              />
            </div>
            ...
            <div className="w-full flex mt-4 gap-3">
              {new Array(3).fill(0).map((_, key) => (
                <div
                  key={key}
                  className="w-[50%] bg-cosmic-primary-color h-[6px] rounded-r-md rounded-l-md "
                />
              ))}
            </div>

            {!faceDetected && (
              <p className="mt-6">Hold on while the system verify your face</p>
            )}
            ...
            {faceDetected && (
              <div className="w-full flex justify-center mt-4">
                <button
                  ...
                  onClick={async () => {
                    if (capturedImage && certificateDetails) {
                      const updatedData = {
                        ...certificateDetails,
                      ...
                      try {
                        ...
                        if (result.status === 200) {
                          store.dispatch(
                            cacheDoctorCertificateAndLicence({
                              certificationDetails: result.data.licenseDetails,
                            }),
                          );
                          setLoading(false);
          ...
          <div className=" absolute top-0 bottom-0 left-0 right-0 bg-black bg-opacity-25 flex justify-center z-[900] items-center p-4">
            <div className="flex flex-col  justify-between items-center bg-white w-[60%] rounded-md shadow-lg shadow-black  z-[200]    max-w-[600px] h-[400px] p-4">
              <div className="flex justify-between items-center w-full relative">
                ...
                <i
                  className="fa fa-times absolute right-2"
                  onClick={() => {
                    if (successfulUpload) {
                      store.dispatch(
                        cacheDoctorCertificateAndLicenceDetailsForUpload({
                ...
                successfulUpload && (
                  <div className="flex w-full justify-center items-center cursor-default">
                    <div className="flex flex-col justify-center w-full items-center  gap-6 p-4  max-w-[400px]">
                      ...
                      <p>
                        You will receive a confirmation email upon eveluation.
                      </p>

                      <p
                        className="mt-3 p-3 bg-cosmic-color-lightBlue text-white w-[100px] text-center rounded-md"
                        onClick={() => {
                          store.dispatch(
                            cacheDoctorCertificateAndLicenceDetailsForUpload({
                              licenceDetails: null,
                          ...
                          setShowUploadComp(false);
                          navigate("/doctor/certification-upload");
                        }}
                      >
                        Ok
                      </p>
              ...
              {loading && (
                <div className="h-full w-full flex flex-col justify-center place-items-center">
                  <label htmlFor="liscence" className=" mr-2 mb-14 ">
                    Uploading... this may take 2-3mins
                  </label>
