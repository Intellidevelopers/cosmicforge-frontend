    ...
    <div className="bg-[#F5F5F5] font-poppins relative z-1 h-screen overflow-y-auto pb-4  ">
      ...
      console.log(certificateDetails)
            if (path === '') {
              ...
              if (certificateDetails && (!certificateDetails.fullName || !certificateDetails.institution || !certificateDetails.certificateNo || !certificateDetails.date || !certificateDetails.certificate || !certificateDetails.photoWithCertification)) {
              ...
              Textextractor.recognize(certificateDetails?.certificate!!, 'eng',
                {}).then(({ data: { text } }) => {
                  const isCertificateValid = text.trim().toLowerCase().includes('Federal republic of nigeria'.trim().toLowerCase()) || text.trim().toLowerCase().includes('Medical & Dental Council Of Nigria'.trim().toLowerCase())
                  setProcessing(false)
            ...
            if (path === 'id') {
              ...
              if (certificateDetails && (!certificateDetails.country || !certificateDetails.docummentType || !certificateDetails.documentId || !certificateDetails.documentHoldName || !certificateDetails.documentImage || !certificateDetails.pictureWithDocument)) {
                setErrorMessage("Please enter the fields completely.")
                return
              }
              ...
              Textextractor.recognize(certificateDetails?.documentImage!!, 'eng',
                {}).then(({ data: { text } }) => {
            ...
            if (path === 'face-id') {
              ...
              if (certificateDetails && (!certificateDetails.fullName || !certificateDetails.institution || !certificateDetails.certificateNo || !certificateDetails.date || !certificateDetails.certificate || !certificateDetails.photoWithCertification || !certificateDetails.country || !certificateDetails.docummentType || !certificateDetails.documentId || !certificateDetails.documentHoldName || !certificateDetails.documentImage || !certificateDetails.pictureWithDocument)) {
            ...
            <div className="w-full flex mt-4 gap-3">

              {
                new Array(3).fill(0).map((_,key) => (
                  ...
                  <div key={key} className="w-[50%] bg-cosmic-primary-color h-[6px] rounded-r-md rounded-l-md " />
                  ...
                  className="min-w-[100px] p-2 bg-cosmic-primary-color font-bold text-white rounded-md" onClick={async () => {
                    ...
                    if (capturedImage && certificateDetails) {
                      ...
                      try {
                        ...
                        setLoading(true)
                        setShowUploadComp(true)
                        const result = await uploadCertificate(updatedData, user.data?.token!!)

                        if (result.status === 200) {

                          store.dispatch(cacheDoctorCertificateAndLicence({ certificationDetails: result.data.licenseDetails}))
                          setLoading(false)
