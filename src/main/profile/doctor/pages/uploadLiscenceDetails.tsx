...
const UploadLiscenceDetails = () => {
  ...
  const liscenseDetails = useSelector(
    (state: RootReducer) =>
      state.doctorCertificateAndLicence.uploadLicenseDetails,
  );
        ...
        <button
          ...
          onClick={async () => {
            ...
            if (path === "") {
              if (
                liscenseDetails &&
                (!liscenseDetails.fullName ||
                  !liscenseDetails.LicenseNumber ||
                  !liscenseDetails.expiration ||
                  (!liscenseDetails.license &&
                    !liscenseDetails.photoWithLicence))
              ) {
            ...
            if (path === "id") {
              if (
                liscenseDetails &&
                (!liscenseDetails.country ||
                  !liscenseDetails.docummentType ||
                  !liscenseDetails.documentId ||
                  !liscenseDetails.documentHoldName ||
                  !liscenseDetails.documentImage ||
                  !liscenseDetails.pictureWithDocument)
              ) {
                setErrorMessage("Please enter the fields completely.");
                return;
              }
