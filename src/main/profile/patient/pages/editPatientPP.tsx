...
const EditPatientPP: React.FC = () => {
  ...
  const [photo, setPhoto] = useState<string | null>(
    `${user.data?.profile?.profilePicture ?? null}`,
  );
  const [fullname, setfullname] = useState<string>(
    `${user.data?.fullName} ${user.data?.lastName}`,
  );
  const [email, setEmail] = useState<string>(user.data?.email!!);
  const [phoneNo, setPhoneNo] = useState<string>(
    `${user.data?.profile?.mobileNo ?? ""}`,
  );
  const [homeAddress, setHomeAddress] = useState<string>(
    `${user.data?.profile?.homeAddress ?? ""}`,
  );
  const [workAddress, setWorkAddress] = useState<string>(
    `${user.data?.profile?.workAddress ?? ""}`,
  );
  ...
  const saveNewChanges = async () => {
    ...
    if (!homeAddress && !phoneNo && !photo && !workAddress && !workAddress) {
      setIsLoading(false);
      setErrorMesage(
        "Either email,homeAddress,phoneNo,workAddress and phoneNumber field is needed to continue",
      );
      return;
    }
    try {
        ...
        <div className=" flex flex-col items-center justify-center pt-[10px] pb-[70px] gap-[10px] p-3  ">
          ...
          <div className="w-[150px] flex flex-col h-[150px] mt-[15px]">
            <img
              ...
              alt="profile-image"
              src={photo ? photo : profileIconTmp}
            />
            <button
              onClick={selectPhoto}
              className="h-[150px] text-[#272EA7] md:opacity-0 opacity-50 hover:opacity-100 cursor-pointer absolute bg-white/50 flex flex-col justify-center items-center rounded-full w-[150px]"
            >
          ...
          <div className="flex flex-col gap-3 w-[100%] md:px-[5%] px-[3%] mt-[15px]">
            <div className="flex flex-col gap-1">
              <input
                className=" h-[50px] border-b-[1px] text-[15px] border-b-black px-2 bg-transparent font-bold outline-none"
                type="text"
          ...
          <button
            ...
            className="rounded-[5px] hover:bg-[#272EA7]/70 bg-[#272EA7] cursor-pointer text-white w-[50%] md:w-[35%] h-[45px] mt-[20px]"
          >
            Save Changes
          </button>
