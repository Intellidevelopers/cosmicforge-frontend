      ...
      <div className="md:w-[75vw] w-full overflow-y-auto">
        ...
        <div className="flex flex-col items-center justify-center pt-[10px] pb-[70px] gap-[10px] p-3  ">
          <div className="bg-white rounded-[5px] w-[100%] md:h-[290px] h-fit shadow-md gap-2 flex md:flex-col flex-row justify-center items-center md:py-3 md:px-3 py-4 shadow-black/10">
            <img
              className="md:w-[160px] md:h-[160px] w-[140px] h-[140px] rounded-full"
              alt="profile-image"
              src={user.data?.profile?.profilePicture ?? profileIconTmp}
            />
            <div className="flex flex-col gap-2 items-center justify-center">
              ...
              <button
                onClick={() => {
                  navigate("/patient/profile/edit-profile");
                }}
                className="h-[30px] text-[15px] w-[120px] rounded-[5px] hover:bg-[#272EA7]/70 bg-[#272EA7] cursor-pointer text-white"
              >
                Edit Profile
              </button>
            </div>
            ...
            <div className="w-[85%] h-[100%] md:pl-[20px] md:border-l-[1px] flex flex-col justify-center items-center md:border-l-gray-300">
              <div className="flex w-[100%] md:items-start gap-1 h-[48%] items-center flex-col justify-center">
                <span className="font-bold text-[17px">
                  {user.data?.profile?.vitalSigns?.gender &&
                  user.data?.profile?.vitalSigns.gender === "male"
                    ? "Mr"
                    : "Mrs"}{" "}
                  {user.data?.fullName} {user.data?.lastName}
                </span>
                ...
                <span className="text-[14px]">
                  {user.data?.profile?.homeAddress ?? "nil"}
                </span>
              </div>
              <div className="flex flex-row w-[100%] md:h-[48%] h-fit justify-between flex-wrap items-center">
                ...
                <div className="w-fit flex flex-col gap-3 p-3 md:text-left text-center">
                  ...
                  <span className="text-[14px] font-bold">
                    {user.data?.profile?.vitalSigns?.dateOfBirth
                      ? user.data?.profile?.vitalSigns.dateOfBirth
                          .toString()
                          .split(" ")[4]
                      : "nill"}
                ...
                <div className="w-fit flex flex-col gap-3 p-3 md:text-left text-center">
                  ...
                  <span className="text-[14px] font-bold">
                    {user.data?.profile?.vitalSigns?.height ?? "nil"}
                  </span>
                </div>
                <div className="w-fit flex flex-col gap-3 p-3 md:text-left text-center">
                  <span className="text-[14px] font-bold text-gray-500">
                  ...
                  <span className="text-[14px] font-bold">
                    {user.data?.profile?.vitalSigns?.weight ?? "nil"}
                  </span>
                </div>
                <div className="w-fit flex flex-col gap-3 p-3 md:text-left text-center">
                  <span className="text-[14px] font-bold text-gray-500">
          ...
          <div className="bg-white rounded-[5px] w-[100%] h-fit shadow-md gap-2 flex flex-col p-3 shadow-black/10">
            ...
            <span className="font-bold text-[15px]">
              {user.data?.profile?.workAddress ?? "nil"}
            </span>
          </div>
          ...
          <div className="flex flex-col gap-2 w-[100%] mt-[10px]">
            <button className="bg-white hover:bg-gray-100 w-[100%] h-[60px] shadow-md flex p-2 flex-row justify-start items-center shadow-black/10">
              ...
              <div
                className="border-l-[2px] pl-[5px] border-l-gray-300 flex flex-row items-center justify-start h-[100%]"
                onClick={() => {}}
              >
