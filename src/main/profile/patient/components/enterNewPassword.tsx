...
const EnterNewPassword: React.FC<props> = ({ forwardFunction, passwordEnteredEarlier }) => {
    ...
    const validateInput =  async () => {
        ...
        if (!(/^(?=.*[a-zA-z])(?=.*[0-9])(?=.*[@£$%^*&?!±|#€∞§¶]).{8,}$/.test(passwordEnteredEarlier))) {
        ...
        if (password !== passwordEnteredEarlier) {
         ...
         try {
          ...
          const result = await  manage_password(user.data?.token!!,{newPassword:password})
        ...
        <div className="flex flex-col items-center justify-start w-[100%] h-[100%]">
            ...
            <div className="h-[330px] flex flex-col justify-start pt-[25px] md:w-[50%] w-[90%] items-center gap-4 bg-white rounded-[10px] mt-[50px]">
                ...
                <div className="flex flex-col md:w-[80%] w-[90%] gap-1">
                    <span>Password</span>
                    <input type="password" value={passwordEnteredEarlier} className="outline-none bg-gray-300 rounded-[7px] h-[45px] px-2 w-[100%]" placeholder="Enter password" />
                </div>
                <div className="flex flex-col md:w-[80%] w-[90%]  gap-1">
                    <span>Confirm Password</span>
                    <input type="password" className="outline-none bg-gray-300 h-[45px] rounded-[7px] px-2 w-[100%]" placeholder="Enter password again" onChange={(e) => {
                        setPassword(e.target.value)
                    }} />
                </div>
                <button onClick={validateInput} className="hover:bg-[#272EA7]/70 bg-[#272EA7] md:w-[80%] w-[90%]  h-[40px] rounded-[7px] mt-[20px] flex flex-row justify-center items-center text-white">Save Changes</button>
